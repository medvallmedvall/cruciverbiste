$("#grille1 td").click(
		function() {
			...
			//si ce n'est pas une definition, ni une case qui est en train d'etre edit̩e
			if (contenuCase.length == 0) {
				horizontal = true;
				selectionnerCase($(this));
			}
			else {
				//si c'est une case entrain d'etre ̩dit̩e
...
$("p").click(
		function() {
			...
			deselectionnnerCase();
			horizontal = true;
			//on recupere la direction du mot dans la balise span cach̩e
...
$("#grille1").keyup(
		function(event) {
			...
			//si on a appuy̩ sur des fleches directionnelles
			if ((c >= 37) && (c <= 40)) {
				//horizontal = true;
				switch(c) {
				case 37:
...
function selectionnerCase(mCase) {
	deselectionnnerCase();
	//si on a selectionn̩ une case qui ne fait pas parti du mots en cours de selection on deselectionne...
