...
/**
 * Home object for domain model class Commentaires.
 * @see hibernate.Commentaires
 * @author Hibernate Tools
 */
public class CommentairesHome {

	private static final Log log = LogFactory.getLog(CommentairesHome.class);
	...
	public void persist(Commentaires transientInstance) {
		log.debug("persisting Commentaires instance");
		try {
	...
	public Commentaires merge(Commentaires detachedInstance) {
		...
		try {
			Commentaires result = (Commentaires) sessionFactory
	...
	public Commentaires findById(java.lang.Integer id) {
		...
		try {
			Commentaires instance = (Commentaires) sessionFactory
					.getCurrentSession().get("hibernate.Commentaires", id);
			if (instance == null) {
	...
	public List findByExample(Commentaires instance) {
		log.debug("finding Commentaires instance by example");
		try {
			List results = sessionFactory.getCurrentSession()
					.createCriteria("hibernate.Commentaires")
